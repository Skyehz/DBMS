...
BOOL DataInsertDlg::OnInitDialog()
{
	...
	//	else
	//		m_listCtl.SetItemText(i, 5, m_rcd[i]);*/
	//}
...
void DataInsertDlg::OnBnClickedOk()
{
	...
	CDataOp dataOption(m_sDBName, m_sTableName);
	...
	if (isOK)
	{
		...
		vector<CDataModel> modelList = dataOp.ReadDataList(m_vFieldList);
		int curOrder = modelList.back().GetId() + 1;
		m_record->SetId(curOrder);
		int code = 0;
		if (m_iOperType == OPERATE_ADD)
		{
			//插入数据
			code = dataOption.AddRecord(*m_record, m_vFieldList);
		}
		else
		{
			//修改数据
			code = dataOption.ModifyRecord(*m_record);
		}
